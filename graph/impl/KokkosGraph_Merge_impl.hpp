...
#include "KokkosGraph_MergePathHints_impl.hpp"
...
struct FlatMerger {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(
    ...
    a_size_type ai = dsr.ai;
    b_size_type bi = dsr.bi;

    // unmanaged views for sequential merge
    // may not necessarily merge a whole chunk at the end of the merge list
    c_size_type nC = KOKKOSKERNELS_MACRO_MIN(chunkSize_, c_.size() - diagonal);
    // may may not be able to read from a or b due to their sizes
    a_size_type nA = KOKKOSKERNELS_MACRO_MIN(chunkSize_, a_.size() - ai);
    b_size_type nB = KOKKOSKERNELS_MACRO_MIN(chunkSize_, b_.size() - bi);

    auto sc = Kokkos::subview(c_, Kokkos::make_pair(diagonal, diagonal + nC));
    auto sa = Kokkos::subview(a_, Kokkos::make_pair(ai, ai + nA));
    auto sb = Kokkos::subview(b_, Kokkos::make_pair(bi, bi + nB));
...
struct HierarchicalMergeHints {
  std::optional<size_t> teamSize;
  std::optional<size_t> threadPathLength;
};
...
struct HierarchicalMerger {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const team_member_type &handle) const {
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(handle, 0, bs.size()),
        [&](const b_size_type i) {
            bs(i) = b_(dsr.bi + i);
        });
    handle.team_barrier();
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(handle, 0, cs.size()),
                         [&](const c_size_type i) { c_(teamDiagonal + i) = cs(i); });
  }
  ...
  static void launch(const ExecSpace &space, const CView &c, const AView &a,
                     const BView &b,
                     const MergePathHints &hints = MergePathHints()) {
    static_assert(
        Kokkos::SpaceAccessibility<ExecSpace,
                                   typename AView::memory_space>::accessible,
        "KokkoGraph::merge_into ExecSpace must be able to access A");
    ...
    // uses shared memory proportional to the team's path length
  #if defined(KOKKOS_ENABLE_CUDA) && defined(KOKKOS_ARCH_VOLTA)
    const size_t threadPathLength = hints.threadPathLength.value_or(4);
    const size_t teamSize   = hints.teamSize.value_or(128);
  #else
    const size_t threadPathLength = hints.threadPathLength.value_or(4);
    const size_t teamSize   = hints.teamSize.value_or(128);
  #endif
    const size_t teamPathLength = threadPathLength * teamSize;
