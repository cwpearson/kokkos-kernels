...
template <typename T, typename Device>
void test_merge_path_global(const std::vector<T> &_a, const std::vector<T> &_b,
                            std::set<StepperParams> expParams) {
  using execution_space = typename Device::execution_space;
  using Policy          = Kokkos::RangePolicy<execution_space>;
  ...
  // place to record all params passed to the stepper function
  Kokkos::View<StepperParams *, Device> actParams("", a.size() + b.size());

  // run
  KokkosGraph::merge_path(a, b, Stepper(a, b, actParams));
  Kokkos::fence();
...
void test_merge_path(const std::vector<T> &a, const std::vector<T> &b,
  ...
  test_merge_path_global<T, Device>(a, b, expParams);
}
