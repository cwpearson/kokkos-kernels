...
/*!
  \brief Team-collaborative parallel merge of sorted \c a and \c b into \c c 
  ...
  No more than c.size() entries will be taken from \c a and \c b combined.
  If \c a or \c b are exhausted, fewer may be taken.
  The number of entries taken from \c a or \c b depends on the values of those entries.
*/
template <typename TeamMember, typename CView, typename AView, typename BView>
KOKKOS_INLINE_FUNCTION void merge_into_team(const TeamMember &handle,
                                            const CView &c, const AView &a,
