...
typename RowMap,
typename Entries,
typename BlockValues,
typename ConstMVView,
typename MVView
>
...
public:
  SparcKernel(const RowMap &A_rp, const Entries &A_ci, const BlockValues &A_d, ConstMVView x, MVView y)
      : A_rp_(A_rp), A_ci_(A_ci), A_d_(A_d), x_(x), y_(y)
      /*adding these */, nrhs_(x_.extent(1)), nblockrows_(A_rp_.extent(0) - 1), blocksz_(y_.extent(0) / nblockrows_),
...
#endif
      typedef Kokkos::View<const Real **, BlockLayout, Kokkos::MemoryTraits<Kokkos::Unmanaged>> ConstBlock;
      ConstBlock b(&A_d_(bsz2_ * j), blocksz_, blocksz_);
      const LO blockcol = A_ci_(j);
      const LO x_start  = blockcol * blocksz_;
      const auto x_lcl  = Kokkos::subview(x_, Kokkos::make_pair(x_start, x_start + blocksz_), irhs);
      for (LO i = 0; i < blocksz_; ++i)
  ...
  run()
  {
    Kokkos::parallel_for(y_.size(), *this);
  }
...
typename Beta,
typename YVector>
void apply_sparc(const Alpha &alpha, const AMatrix &a, const XVector &x, const Beta &beta, const YVector &y) {
